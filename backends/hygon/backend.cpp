...
flagdnnBackendResult_t plugin_call(Function &&function) noexcept {
  ...
  try {
    ...
    current_error = "unknown Hygon backend error";
    return FLAGDNN_BACKEND_RESULT_INTERNAL_ERROR;
  }
}
...
flagdnnBackendResult_t create_context(std::int32_t device_ordinal,
  ...
  return plugin_call([&] {
    ...
    *context = nullptr;
    std::unique_ptr<HygonContext> result =
        std::make_unique<HygonContext>(device_ordinal);
    *context = result.release();
  });
}

void destroy_context(void *context) noexcept {
  delete static_cast<HygonContext *>(context);
}
...
get_target_fingerprint(void *context, char *buffer, std::size_t buffer_size,
  ...
  return plugin_call([&] {
    ...
    const std::string &target =
        static_cast<HygonContext *>(context)->target_fingerprint();
  ...
  return plugin_call([&] {
    ...
    *workspace_size = 0;
    ...
    const EngineBuildContext build_context =
        static_cast<HygonContext *>(context)->engine_build_context();
...
flagdnnBackendResult_t execute(void *executable, void *native_stream,
  ...
  return plugin_call([&] {
    ...
    static_cast<ExecutionEngine *>(executable)
        ->execute(reinterpret_cast<hipStream_t>(native_stream), bindings,
