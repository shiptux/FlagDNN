#!/usr/bin/env python3
"""Fresh-cache offline BiSheng contract for Ascend RMSNorm."""
...
if not __debug__:
    raise RuntimeError("offline RMSNorm matrix requires assertions enabled")
...
OUT = Path(tempfile.mkdtemp(prefix="flagdnn-rmsnorm-offline."))
...
from tools.test_ascend_compiler_contract import TARGET, _compile, _rmsnorm_request
...
expected_abi = [
    ...
    {"index": 4, "name": "inv_variance_ptr", "type": "pointer"},
    {"index": 5, "name": "n_elements", "type": "i32"},
]
pointer_token = {
    "float32": "*fp32:16",
    "float16": "*fp16:16",
...
for case_name, graph_request, candidate_count in cases:
    ...
    assert len(stages) == 1
    stage = stages[0]
    assert stage["operation"] == "rmsnorm"
    assert stage["kernel_family"] == "rmsnorm"
    assert stage["kernel"]["source"] == "normalization.py"
    assert stage["kernel"]["entry_point"] == "rmsnorm_persistent_kernel"
    ...
    for candidate in candidates:
        ...
        assert payload["entry_point"] == "rmsnorm_persistent_kernel"
        ...
        assert ttir.count("scf.while") >= 3
        assert "tt.reduce" in ttir
        assert ttir.count("tt.store") >= 2
        completed += 1
        print(
...
print(
    f"PASS RMSNorm offline matrix completed={expected_total} out={OUT}",
    flush=True,
)
