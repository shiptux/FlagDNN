...
flagdnnBackendResult_t plugin_call(Function&& function) noexcept {
  ...
  try {
    ...
    current_error = "unknown Ascend backend error";
    return FLAGDNN_BACKEND_RESULT_INTERNAL_ERROR;
  }
}
...
flagdnnBackendResult_t create_context(std::int32_t device_ordinal,
  ...
  return plugin_call([&] {
    ...
    *context = nullptr;
    ...
    std::unique_ptr<AscendContext> result =
        std::make_unique<AscendContext>(device_ordinal);
    ensure_process_healthy();
...
void destroy_context(void* context) noexcept {
  delete static_cast<AscendContext*>(context);
}
...
flagdnnBackendResult_t get_target_fingerprint(
  ...
  return plugin_call([&] {
    ...
    const std::string& target =
        static_cast<AscendContext*>(context)->target_fingerprint();
  ...
  return plugin_call([&] {
    ...
    *workspace_size = 0;
    ...
    const EngineBuildContext build_context =
        static_cast<AscendContext*>(context)->engine_build_context();
    ContextGuard context_guard(build_context.context);
...
flagdnnBackendResult_t execute(
  ...
  return plugin_call([&] {
    ...
    static_cast<ExecutionEngine*>(executable)->execute(native_stream,
                                                          bindings,
                                                          binding_count,
                                                          workspace,
                                                          workspace_size);
...
const flagdnnBackendApiV3 api = {
    sizeof(flagdnnBackendApiV3),
    FLAGDNN_BACKEND_ABI_VERSION_V3,
...
extern "C" FLAGDNN_BACKEND_EXPORT const flagdnnBackendApiV3*
flagdnnBackendGetApiV3(void) {
  return &flagdnn::ascend::api;
}
