...
class TransposeExecutable final : public ExecutableBase {
 public:
  explicit TransposeExecutable(const BenchmarkCase& specification) {
    ...
    auto output = graph_->transpose(
        input,
        fe::graph::Transpose_attributes()
            .set_name("transpose")
            .set_permutation(specification.transpose.permutation));
...
std::unique_ptr<BenchmarkExecutable> build_transpose(
    const BenchmarkCase& specification) {
  return std::make_unique<TransposeExecutable>(specification);
}
